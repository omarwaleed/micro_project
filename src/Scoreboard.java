import java.util.*;

 public class Scoreboard {
   static Scoreboard sb;
   Hashtable<String,Integer > registerStatus;//contains names or registers and no  of entries  //modified
   static Entry[] rob;  // reorder buffer is an array of entries  //modified to static
   int head;  // rob head pointer
   int tail;  // rob tail pointer
	ArrayList<FU>functionalUnits; // keep all the reservation stations
    Hashtable<Instruction,String>instructions; // each instruction is mapped to its current phase
   static  Processor p;
   Scoreboard(Processor pp){//modified
	   registerStatus = new Hashtable<String,Integer>();
	   instructions=new Hashtable<Instruction,String>();
	   int size=10;
	   rob = new Entry[size];
	   for (int i = 0; i <size ; i++) {
		   rob[i]=new Entry();
		
	}
	   head = tail = 1;
       functionalUnits = new ArrayList<FU>();
       p=pp;
       ////
       FU x1=new FU("load", "load1", 1);
       FU x2=new FU("mul", "mul1", 3);
       FU x3=new FU("add", "add1", 2);
       functionalUnits.add(x1);
       functionalUnits.add(x2);
       functionalUnits.add(x3);
       
       
       ////
   }
   public void clearRegStatus(String reg){
	   registerStatus.put(reg, null);
   }
   public void print_scoreboard() {//modified
	   System.out.println("the ROP is: "+"\n");
	   for (int i = 0; i < rob.length; i++) {
		   System.out.println(rob[i].toString());
		
	}
	   System.out.println();
	   System.out.println("th Reservation staions are  : "+"\n"); 
	   for (int i = 0; i < functionalUnits.size(); i++) {
		   System.out.println(functionalUnits.get(i));
		
	}
	   System.out.println();
	   System.out.println("Register status   is: "+"\n");
	   if(registerStatus.toString()!=null){
		   System.out.println(registerStatus.toString());
	   }
		   System.out.println();
		   System.out.println("the instructions states are "+"\n");
		   System.out.println(instructions.toString());
		
	
		
	}
   
   public Entry [] get_rob(){//modified
	   return rob;
   }
   public static Scoreboard getInstance() {  // use this method to get access the score board
	   if (sb == null) 
		   sb = new Scoreboard(null);
	   return sb;
   }
  public int increment(int i) {  // use this method to increment either the head or the tail and pass to it the int you want to increment
	  if (i == 9) 
		  return 0;
	      return ++i;
  }
  public boolean robFull() {  // tested
	 for(int i = 0; i < rob.length;i++) {
		 if (!rob[i].occupied)
			 return false;
	 }
	   return true;
  }
  public FU getFU(String type){
	  for (int i = 0; i < functionalUnits.size(); i++) {
		  if(functionalUnits.get(i).type.equals(type)){
			  return functionalUnits.get(i);
		  }
	}
	  return null;
	  
  }
  public void insertFU(String type, String name, int latency,int d) { //tested
      FU fu = new FU(type,name,latency);
      functionalUnits.add(fu);
  }
  public boolean fullFuncUnit(String type) { // tested
      for(int i = 0; i<functionalUnits.size();i++) {
          FU fu = functionalUnits.get(i);
          if (!fu.busy && fu.type.equalsIgnoreCase(type))
              return false;
      }
      return true;
  }
    public void insertROB(Entry entry) { //tested
        rob[tail] = entry;
       tail = increment(tail);
    }
    public void removeFromROB() { // tested
        rob[head] = null;
       head = increment(head);
    }
    public static void main(String[]args) {
        Scoreboard sb = Scoreboard.getInstance();
        Entry e1 = new Entry("add","F6");
       // System.out.println(e1);
        sb.insertROB(e1);
        sb.insertROB(e1);
        System.out.println("head: "+ sb.head + " tail " + sb.tail);
        sb.removeFromROB();
        System.out.println("after removing: head: "+ sb.head + " tail " + sb.tail);
     // sb.insertFU("FP","addd1",3);
//        sb.functionalUnits.get(0).busy = true;
       sb.insertFU("int","add1",1,0);
//        System.out.println("FU FP full: "+ sb.fullFuncUnit("FP"));
//        System.out.println("FU int full: "+ sb.fullFuncUnit("int"));
    }
   
}