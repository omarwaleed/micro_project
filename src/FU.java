public class FU {
	...
	public void execute() {
		//System.out.println();
		Entry entry = Scoreboard.rob[dest]; // el mkan elly hakhzn feeh el
											// result
		val = null;
		if (op.equalsIgnoreCase("load")) {
			...
			if (qj == null && qk == null) {// they are in the registers
				int result = Scoreboard.p.registers.get(vj)
						+ Scoreboard.p.registers.get(vk);
			...
			if (qj == null && qk == null) {// they are in the registers
				int result = ~(Scoreboard.p.registers.get(vj) & Scoreboard.p.registers
						.get(vk));
				val = String.valueOf(result);
				...
				if (qj != null) {// i have to get vj from the ROB
					...
					result = ~(Integer.parseInt(entry1.value) & Scoreboard.p.registers
							.get(vk));

				} else {// qk is not null
					Entry entry1 = Scoreboard.rob[Integer.parseInt(qk)];
					result = ~(Scoreboard.p.registers.get(vj) & Integer
